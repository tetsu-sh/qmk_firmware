...
_qmk_install() {
    ...
    if apt-get install --simulate --quiet --yes picolibc-riscv64-unknown-elf gcc-riscv64-unknown-elf binutils-riscv64-unknown-elf > /dev/null 2>&1; then
        apt-get --quiet --yes install picolibc-riscv64-unknown-elf \
